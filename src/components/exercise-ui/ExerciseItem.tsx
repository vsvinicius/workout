...
export default function ExerciseItem({ exercise }: { exercise: Exercise }) {
  ...
  return (
    <div className="relative w-[96%]">
      {isWaitingCountdown && (
        <Countdown onEnd={() => setIsWaitingCountdown(false)} />
      )}
      <Card
        elevation={10}
        className={cn(
          'my-3 h-fit min-h-32 w-full border border-solid border-white-light bg-background-light px-4 py-8',
        )}
        sx={{
          transition: 'min-height 0.3s linear',
          ...(isWaitingCountdown && { opacity: 0.3 }),
